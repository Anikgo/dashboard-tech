...
import { Factory, Clock, Users, Truck, TrendingUp, Activity, ImageIcon, CheckCircle } from "lucide-react";
...
export default function OperationsPage() {
  ...
  const fetchResolvedAlerts = async () => {
    try {
      const response = await axios.get('http://localhost:3001/api/resolved-alerts/operations');
  ...
  const handleResolve = async (id: string, dataType: string, alertData: any) => {
    ...
    try {
      ...
      switch (dataType) {
        case 'loitering':
          ...
          break;
        case 'idle_machinery':
          setIdleMachineryData(prev => prev.filter(item => item._id !== id));
          break;
        case 'attendance':
            ...
            <motion.div variants={itemVariants}>
              <Card className="border border-gray-200 shadow-lg w-full bg-white">
                <CardHeader className="p-4 pb-2">
                  ...
                  <CardTitle className="text-lg font-semibold text-guardai-darkgray">Resolved Alerts</CardTitle>
                  <p className="text-sm text-guardai-gray">History of all resolved alerts and incidents</p>
                </CardHeader>
