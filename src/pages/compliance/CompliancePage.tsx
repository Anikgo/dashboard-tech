...
export default function CompliancePage() {
  ...
  const fetchResolvedAlerts = async () => {
    try {
      const response = await axios.get('http://localhost:3001/api/resolved-alerts/compliance');
            ...
            <motion.div variants={itemVariants}>
              <Card className="border-2 shadow-lg w-full bg-white">
                <CardHeader className="p-4 pb-2">
                  ...
                  <CardTitle className="text-lg font-semibold text-guardai-darkgray">Resolved Compliance Alerts</CardTitle>
                  <p className="text-sm text-guardai-gray">History of all resolved PPE compliance violations</p>
                </CardHeader>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="bg-guardai-red/5">
                          ...
                          <TableHead className="text-xs text-guardai-darkgray">Violation</TableHead>
                      ...
                      <TableBody>
                        {resolvedAlerts.map((alert, idx) => (
                          <TableRow key={idx} className="hover:bg-gray-50">
                            ...
                            <TableCell className="text-xs">{alert.originalData.person_id}</TableCell>
                        ...
                        <TableBody>
                          {feature.data.violations.map((item, index) => (
                            <TableRow key={index} className="hover:bg-gray-50">
                              ...
                              <TableCell className="text-xs">
                                {feature.id === "ppe-compliance" && "Hairnet Missing"}
                                {feature.id === "phone-compliance" && "Phone Usage"}
                                {feature.id === "sleep-compliance" && "Sleeping on Duty"}
                              </TableCell>
                              <TableCell className="text-xs">
                              ...
                              <TableCell className="text-xs">{item.zone || item.camera_id}</TableCell>
                              <TableCell className="text-xs">
                                <a
                              ...
                              <TableCell className="text-xs">
                                <button
                                  onClick={() => {
                                    const alertType = feature.id === "ppe-compliance" ? "ppe_compliance" : 
                                                   feature.id === "phone-compliance" ? "phone" : "sleeping";
